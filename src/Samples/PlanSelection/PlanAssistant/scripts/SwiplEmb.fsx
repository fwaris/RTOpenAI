(*
Call swipl prolog via its C api (which meant for embedding swipl in other programs and other interop scenarios)
Tested only on macos. See also SwiplLib project
*)
...
let run (predicates:string) (goal:string) =               
    PL_initialise(2,[|"libswipl";"-q"|]) |> check
    loadPrologFile clausesFile
    let t_pred = PL_new_term_ref()
    let t_goal = PL_new_term_ref()
