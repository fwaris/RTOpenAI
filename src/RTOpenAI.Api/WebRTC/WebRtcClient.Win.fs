...
type WebRtcClientWin() =
    ...
    let audioDelegate (pc: RTCPeerConnection) =
        EncodedSampleDelegate(fun duration bytes -> pc.SendAudio(duration, bytes))
    ...
    let addAudio (pc: RTCPeerConnection) =
        ...
        let audioTrack =
            new MediaStreamTrack(AudioCommonlyUsedFormats.OpusWebRTC, MediaStreamStatusEnum.SendRecv)
        ...
        pc.add_OnAudioFormatsNegotiated (fun afs ->
            ...
            Log.info
                $"Audio format negotiated {af.FormatName} channels:{af.ChannelCount} rate:{af.ClockRate} rtpRate:{af.RtpClockRate} {af.FormatID}."
    ...
    interface IWebRtcClient with
        ...
        member _.SetMicrophoneEnabled(enabled: bool) =
            Log.info $"pc: microphone enabled request ignored on Windows: enabled={enabled}"
#endif
